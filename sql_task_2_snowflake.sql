CREATE OR REPLACE PROCEDURE test2_calls(start_date DATE)
RETURNS TABLE(call_text STRING)
LANGUAGE PYTHON
RUNTIME_VERSION = '3.9'
...
AS
$$
def test2_calls(session, start_date):
    prev_date = start_date
    data_test2 = session.create_dataframe([
    ...
    call_test2 = []
    for row in data_test2.collect():
        param1 = row['PARAM1']
        param2 = row['PARAM2']
        sql = f"Call test2('{param1}', '{param2}', '{prev_date}')"
        session.sql(sql).collect()
        call_test2.append((sql,))
        prev_date = param1
